...
public class EditManager : MonoBehaviour
{
    ...
    // 출발 정보
    GUIStyle startInfoStyle = new GUIStyle();
    StringBuilder _startGimmickInfoText = new StringBuilder();
    ...
    void Start()
    {
        startInfoStyle.normal.textColor  = Color.white;
        startInfoStyle.fontSize = 11;
    ...
    void OnGUI()
    {
        ...
        else if (windowMode == EditWindowMode.START_INFO)
        {
            GUIContent guiContent = new GUIContent();
            guiContent.text   = _toolState;
            windowRectSetting = GUI.Window(0, windowRectSetting, windowStartInfo, guiContent);
        }
    ...
    public void ResetTool()
    {
        ...
        //인게임 아이템 오브젝트 제거
        GameUIManager.instance.DestroyIngameItemUI();
    ...
    void SetEditTap()
    {
        ...
        GUI.Label(new Rect(x + 180, y, 50, 20), "부스트 단계");
        if (int.TryParse(GUI.TextField(new Rect(x + 230, y, 30, 25), boostStep.ToString()), out parseInt))
        {
            if (boostStep != parseInt)
            {
                boostStep = parseInt > 4 ? 4 : parseInt;
            }
        }
        ...
        for (int i = 0; i < System.Enum.GetNames(typeof(TARGET_TYPE)).Length; i++)
        {
            ...
            x = SetLabelAndGetXPos(x, y, GetTargetTypeText((TARGET_TYPE)i), 40);
        }
    ...
    float SetLabelAndGetXPos(float x, float y, string text, int width = 35)
    {
        GUI.Label(new Rect(x, y + 20, width, 25), text);
        return x + 25f;
    }
    ...
    IEnumerator CoSave(string path)
    {
        ...
        //불필요한 인게임 배경 및 UI 감춤
        GameUIManager.instance.SetActiveHideObjectsAtMapSave(false);
        ...
        //todo : 
        #region 스크린 샷 로컬에 저장 (출발 정보)
        var extraImages = new List<InGameFileHelper2.ExtraImageInfo>();
        
        //탭을 출발 정보로 옮김.
        SetWindowMode(EditWindowMode.START_INFO);
        ...
        File.WriteAllBytes(Application.persistentDataPath + "/" + _stringFileName + "_0.png", bytes2);

        extraImages.Add(new InGameFileHelper2.ExtraImageInfo(_stringFileName + "_0.png", bytes2));
        #endregion
        
        //불필요한 인게임 배경 및 UI 활성화
        GameUIManager.instance.SetActiveHideObjectsAtMapSave(true);
        //탭을 맵 화면으로 옮김.
        SetWindowMode(EditWindowMode.HOME);
        

        #region 신규 저장소에 저장
        IEnumerator e = InGameFileHelper2.WWW_Upload(_stringFileName + ".xml", bytes, extraImages, GetGimmickString(), GetTargetString(), GetModeString(), GetDescriptionString(), GetAuthorString(), _startGimmickInfoText.ToString());
        while (e.MoveNext())
            yield return e.Current;
    ...
    private struct StartGimmickInfo
    {
        public StartGimmickInfo(BlockType type, string info)
        {
            this.type = type;
            this.info = info;
        }
    ...
    private List<StartGimmickInfo> _startGimmickInfos = new List<StartGimmickInfo>();
    
    /// <summary>
    /// 단계 석판의 경우 출발 기믹 데이터가 아니지만, 레벨팀 요청으로 출발 기믹 데이터에 포함시킴.
    /// </summary>
    private float WriteCountCrackInfo(float x, float y)
    {
        ...
        {
            var countCrackInfo = ManagerBlock.instance.stageInfo.listDecoInfo[gimmickIndex] as GimmickInfo_CountCrack;
            if (countCrackInfo == null)
            {
                return y;
            }

            foreach (var appleInfo in countCrackInfo.listAppleInfo)
            {
                if (appleInfo.count <= 0)
                {
                    continue;
                }

                var result = $"[단계석판] 인덱스:{appleInfo.crackIndex},사과:{appleInfo.index},카운트:{appleInfo.count}";
                GUI.Label(new Rect(x, y, 800, 20), result, startInfoStyle);
                _startGimmickInfoText.AppendLine(result);
                y += 15;
            }
        }
    ...
    /// <summary>
    /// [SMS 출발 기믹 데이터] 출발, 생성기에서 생성되는 기믹 정보 (기믹별 생성 확률 설정)
    /// </summary>
    private void SetStartGimmickInfo(float x, float y)
    {
        if (ManagerBlock.instance.stageInfo.ListStartInfo == null)
        {
            return;
        }

        // 확률 값이 있더라도 출발/생성기에 설정되어있지 않을 경우 체크를 위해 출발/생성기에 설정된 기믹 리스트
        var blockList = GetGenerativeGimmickList();
        
        foreach (var startInfo in ManagerBlock.instance.stageInfo.ListStartInfo)
        {
            var builder = new StringBuilder();
            var type    = (BlockType)startInfo.type;

            if (blockList.Contains(type) == false)
            {
                continue;
            }
            ...
            {
                builder.Append($"확률:{startInfo.probability.ToString()}");
            }
            else
            {
                continue;
            ...
            if (startInfo.max_display_Count > 0)
            {
                if (builder.Length > 0)
                {
                    builder.Append("/");
                ...
                builder.Append($"화면수:{startInfo.max_display_Count.ToString()}");
            }

            if (startInfo.max_stage_Count > 0)
            {
                if (builder.Length > 0)
                {
                    builder.Append("/");
                ...
                builder.Append($"전체:{startInfo.max_stage_Count.ToString()}");
            }

            if (startInfo.minTurn > 0)
            {
                if (builder.Length > 0)
                {
                    builder.Append("/");
                ...
                builder.Append($"턴간:{startInfo.minTurn.ToString()}");
            }

            if (type != BlockType.START_Bomb && type != BlockType.START_Rainbow &&
                // 첫번째 칸에 1이상만 있으면 그 외에는 아무기능을 하지않는 기믹의 경우 표기 제외(열쇠, 연쇄폭탄, 거미)
                type != BlockType.KEY && type != BlockType.BLOCK_BLACK && type != BlockType.SPACESHIP)
            {
                var temp    = "";
                var hasData = false;

                for (var i = 0; i < startInfo.countProb.Length; i++)
                {
                    if ((type == BlockType.START_Line || type == BlockType.PAINT || type == BlockType.PEA || type == BlockType.PEA_BOSS) && i == 2)
                    {
                        continue;
                    }

                    if (startInfo.countProb[i] > 0)
                    {
                        hasData = true;
                    }

                    temp += $"{(i != 0 ? "," : "")}{startInfo.countProb[i].ToString()}";
                }
                ...
                if (hasData)
                {
                    ...
                    builder.Append($"{GetTextForTypeC(type)}:{temp}");
                }
            }

            if (type == BlockType.PAINT || type == BlockType.BLOCK_DYNAMITE || type == BlockType.GROUND_BOMB || type == BlockType.COIN_BAG || type == BlockType.GROUND_APPLE ||
                type == BlockType.WATERBOMB)
            {
                var temp    = "";
                var hasData = false;
                ...
                {
                    if (startInfo.timeCount[i] > 0)
                    {
                        hasData = true;
                    }

                    temp += $"{(i != 0 ? "," : "")}{startInfo.timeCount[i].ToString()}";
                }
                ...
                if (hasData)
                {
                    ...
                    builder.Append($"{GetTextForTypeTC(type)}:{temp}");
                }
            }

            if (type != BlockType.GROUND_BLOCKBLACK && type != BlockType.GROUND_ICE_APPLE)
            {
                if (startInfo.type == (int)BlockType.ICE)
                {
                    ...
                    {
                        if (startInfo.iceProb[i] > 0)
                        {
                            hasData = true;
                        }

                        temp += $"{(i != 0 ? "," : "")}{startInfo.iceProb[i].ToString()}";
                    }
            ...
            {
                if (ManagerBlock.instance.stageInfo.isFixedIceAdjust == 1)
                {
                    if (builder.Length > 0)
                    {
                        builder.Append("/");
                    ...
                    builder.Append("고정얼음화면수미포함");
                }
            }
            else if (type == BlockType.BLOCK_BLACK)
            {
                if (ManagerBlock.instance.stageInfo.isBlockBlackAdjust == 1)
                {
                    if (builder.Length > 0)
                    {
                        builder.Append("/");
                    ...
                    builder.Append("화면수미포함");
                }
            }
            else
            {
                if (ManagerBlock.instance.stageInfo.listBlockAdjust.Any(a => a == (int)type))
                {
                    if (builder.Length > 0)
                    {
                        builder.Append("/");
            ...
            var result = $"[{GetBlockName_ByBlockType(type)}] {builder}";
            _startGimmickInfos.Add(new StartGimmickInfo(type, result));
        }
    }

    /// <summary>
    /// [SMS 출발 기믹 데이터] 생성기 인덱스별로 설정된 기믹 종류(+컬러) 데이터
    /// </summary>
    private float WriteGeneratorInfo(float x, float y)
    {
        ...
        foreach (var generatorBlockInfo in dicGeneratorBlockInfo)
        {
            var builder        = new StringBuilder();
            var generatorIndex = generatorBlockInfo.Key;
            ...
            {
                if(_startGimmickInfos.FindIndex(x => x.type == listCanMakeBlock[i]) == -1)
                {
                    continue;
                }
                ...
                builder.Append($"{GetBlockName_ByBlockType(listCanMakeBlock[i])}");

                if (listCanMakeBlock[i] == BlockType.PAINT)
                {
                    var paintColor = new StringBuilder();
                    if (dicGeneratorBlockInfo[generatorIndex].dicCanMakeColor.ContainsKey(BlockType.PAINT))
                    {
                        //저장되어 있던 생성가능 컬러 데이터 가져옴
                        int colorData_cur = dicGeneratorBlockInfo[generatorIndex].dicCanMakeColor[BlockType.PAINT];

                        for (int c = 0; c < 5; c++)
                        {
                            int colorIdx = (c + 1);
                            if ((colorData_cur & (1 << colorIdx)) != 0)
                            {
                                ...
                                paintColor.Append($"{GetColorName((BlockColorType)colorIdx)}");
                            }
                        }
                    }
                    ...
                    if (paintColor.Length > 0)
                    {
                        builder.Append($"({paintColor.ToString()})");
                    }
                }
            }
            ...
            int canMakeColor = dicGeneratorBlockInfo[generatorIndex].dicCanMakeColor.ContainsKey(BlockType.NORMAL)
                ? dicGeneratorBlockInfo[generatorIndex].dicCanMakeColor[BlockType.NORMAL]
                : int.MaxValue;

            for (int i = 0; i < generatorBlockColor.Length; i++)
            {
                generatorBlockColor[i] = ((canMakeColor & (1 << (i + 1))) != 0);
                if (generatorBlockColor[i])
                {
                    if (builder.Length > 0)
                    {
                        builder.Append(",");
                    ...
                    var cType = (BlockColorType)(i + 1);
                    builder.Append($"{GetColorName(cType)}");
                }
            }

            if (builder.Length > 0)
            {
                var result = $"[생성기{generatorBlockInfo.Key}] {builder.ToString()}";
                _startGimmickInfoText.AppendLine(result);

                if (result.Length > 70)
                {
                    GUI.Label(new Rect(x, y, 800, 20), GetReplaceColorName(result.Substring(0, 70)), startInfoStyle);
                    y += 15;
                    GUI.Label(new Rect(x, y, 800, 20), GetReplaceColorName(result.Substring(70)), startInfoStyle);
                }
                else
                {
                    GUI.Label(new Rect(x, y, 800, 20), GetReplaceColorName(result), startInfoStyle);
                }
    ...
    private string GetReplaceColorName(string original)
    {
        return original
            .Replace("분홍", GetBlockColorName_WithColorType(BlockColorType.A))
            .Replace("갈색", GetBlockColorName_WithColorType(BlockColorType.B))
            .Replace("노랑", GetBlockColorName_WithColorType(BlockColorType.C))
            .Replace("주황", GetBlockColorName_WithColorType(BlockColorType.D))
            .Replace("파랑", GetBlockColorName_WithColorType(BlockColorType.E));
    }

    private string GetColorName(BlockColorType colorType)
    {
        switch (colorType)
        {
            case BlockColorType.A:
    ...
    private string GetTextForTypeC(BlockType type)
    {
        switch (type)
        {
            case BlockType.APPLE:
                return "턴 별 확률";
            case BlockType.START_Line:
                return "가로세로";
            default:
                return "단계별 확률";
        }
    ...
    private string GetTextForTypeTC(BlockType type)
    {
        switch (type)
        {
    ...
    /// <summary>
    /// 출발, 생성기에서 생성 가능한 기믹 리스트 반환
    /// </summary>
    private List<BlockType> GetGenerativeGimmickList()
    {
        List<BlockType> resultList = new List<BlockType>();
        foreach (BlockInfo blockInfo in ManagerBlock.instance.stageInfo.ListBlock)
        {
            foreach (var deco in blockInfo.ListDeco)
            {
                ...
                {
                    resultList.AddRange(GetStartBlockName(deco));
                }
            }
        }
    ...
    private List<BlockType> GetStartBlockName(DecoInfo decoInfo)
    {
        List<BlockType> resultList = new List<BlockType>();
        foreach (BlockType bType in Enum.GetValues(typeof(BlockType)))
        {
            int index = (int)bType;
            if ((index <= 32 && ((decoInfo.index & (1 << index))        != 0)) ||
                (index > 32  && ((decoInfo.type  & (1 << (index - 32))) != 0)))
            {
    ...
    // TODO
    private void windowStartInfo(int windowID)
    {
        float y = 5f;
        float x = 5f;
        ...
        _startGimmickInfoText.Clear();
        _startGimmickInfos.Clear();

        SetStartGimmickInfo(x, y);
        
        y = WriteCountCrackInfo(x, y);
        
        y = WriteGeneratorInfo(x, y);

        foreach (var info in _startGimmickInfos)
        {
            _startGimmickInfoText.AppendLine(info.info);
            
            if (info.info.Length > 70)
            {
                GUI.Label(new Rect(x, y, 800, 20), info.info.Substring(0, 70), startInfoStyle);
                y += 15;
                GUI.Label(new Rect(x, y, 800, 20), info.info.Substring(70), startInfoStyle);
            }
            else
            {
                GUI.Label(new Rect(x, y, 800, 20), info.info, startInfoStyle);
            }
            y += 15;
        }
    ...
    private string GetModeText()
    {
        switch (ManagerBlock.instance.stageInfo.gameMode)
        {
            ...
            case (int)GameMode.DIG:
                return "<color=#ff00ff>땅파기</color>";
            case (int)GameMode.LAVA:
                return "<color=#ff00ff>용암</color>";
    ...
    void windowAdvance(int windowID)
    {
        ...
        GUI.Label(new Rect(350, 130, 60, 20), "랜덤씨드");
        int.TryParse(GUI.TextField(new Rect(410, 130, 30, 20), inGameSeed.ToString()), out inGameSeed);
    ...
    */
    ...
    string GetStartBlockString2(List<int> listCanMakeBlock)
    {
        ...
        block += GetStartBlockName(listCanMakeBlock, " 벌집", (int)BlockType.FIRE_WORK, 1);
    ...
    */
    ...
    private void MakeBlockRandomPos(int maxCount, BlockType type, int blockCount, bool isMakeActiveBoard = true)
    {
        ...
        {
            int inX = board.indexX;
            int inY = board.indexY;

            if (board.Block != null)
            {   
                if (board.Block.type == type)
                {
            ...
            if (inX < minX || inX >= maxX || inY < minY || inY >= maxY)
                continue;

            if (board.IsActiveBoard != isMakeActiveBoard)
                continue;

            if (board.IsHasScarp == true && isMakeActiveBoard == true)
                continue;

            listCanMakeBoard.Add(board);
        }
