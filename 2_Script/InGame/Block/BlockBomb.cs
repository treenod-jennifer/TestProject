...
public class BlockBomb : MonoBehaviour
{
    ...
    {
        if (AdventureManager.instance.TestBombType == 0)
        {
            ...
            GameObject circleObj = InGameEffectMaker.instance.MakeRainbowCircleEffect(Vector3.zero);
            ...
            GameObject circleObj2 = InGameEffectMaker.instance.MakeRainbowCircleEffect2(Vector3.zero);
        ...
        else
        {
            ...
            for (int i = 0; i < rainbowPangList.Count; i++)
            {
                ...
                {
                    waitBombTimer++;
                    InGameEffectMaker.instance.MakeRainbowLineAdventureBomb(Vector3.zero, rainbowPangList[i]._transform.position, rainbowPangList[i], _uniqueIndex, BlockBombType.HALF_BOMB, i * 0.02f);
            ...
            bombTimer = 0;
            while (bombTimer < 1 + 0.1f + waitBombTimer - BOMB_DESTROY_TIME)
            {
                bombTimer += Global.deltaTimePuzzle;
                yield return null;
            }
    ...
    IEnumerator RainbowPangSameLine(int lineY)
    {
        ...
        while (listBlock.Count > 0)
        {
                ...
                block.IsBombBlock())
            {
                ...
                if (tempBoard.HasDecoCoverBlock() == false && (block.blockDeco == null || block.blockDeco.IsInterruptBlockSelect() == false))
                {
                    ManagerBlock.instance.comboCount++;
                    InGameEffectMaker.instance.MakeAdventureCombo(block._transform.position, "Bomb\nCombo " + ManagerBlock.instance.comboCount.ToString());// + new Vector3(0, -0.06f, 0)
                }
            }
    ...
    IEnumerator PangRainbowXBomb2(bool ingameRainbowStick = false)
    {
        ...
        if(ingameRainbowStick == true)
        {
           ...
           BlockMaker.instance.MakeBombBlock(_block, indexX, indexY, BlockBombType.HALF_BOMB, _block.colorType, false);
    ...
    IEnumerator PangLine(int offX, int offY)
    {
        ...
        while (timer < 0.4f && GameManager.instance.SkipIngameClear == false)
        {
            timer += Global.deltaTimePuzzle;
            yield return null;
        }
        ...
        while (bombTimer < BOMB_DESTROY_TIME && GameManager.instance.SkipIngameClear == false)
        {
            bombTimer += Global.deltaTimePuzzle;
            yield return null;
        }
    ...
    IEnumerator PangCircleBomb(float waitTime = 0.3f)
    {
        ...
        float timer = 0f;
        while (timer < 0.2f && GameManager.instance.SkipIngameClear == false)
        {
            timer += Global.deltaTimePuzzle;
            yield return null;
        }
    ...
    IEnumerator PangRainbowBomb()
    {
        ...
        {
            bombTimer = 0;
            while (bombTimer < 0.2f && GameManager.instance.SkipIngameClear == false)
            {
                bombTimer += Global.deltaTimePuzzle;
                yield return null;
            }
            ...
            bombTimer = 0;
            while (bombTimer < 0.3f && GameManager.instance.SkipIngameClear == false)
            {
                bombTimer += Global.deltaTimePuzzle;
                yield return null;
            }
        ...
        for (int i = 0; i < rainbowPangList.Count; i++)
        {
            ...
            if (GameManager.instance.SkipIngameClear)
            {
                rainbowPangList[i]._pangRemoveDelay = 0.0f;
                rainbowPangList[i].IsSkipPang = false;
                rainbowPangList[i].isRainbowBomb = false;
                rainbowPangList[i].BlockPang(_uniqueIndex, BlockColorType.NONE, false);
            }
            else
            {
            ...
            bombTimer = 0;
            while (bombTimer < 0.02f && GameManager.instance.SkipIngameClear == false)
            {
                bombTimer += Global.deltaTimePuzzle;
                yield return null;
            }
        ...
        while (bombTimer < RAINBOW_DESTROY_TIME && GameManager.instance.SkipIngameClear == false)
        {
            bombTimer += Global.deltaTimePuzzle;
            yield return null;
        }
    ...
    IEnumerator CoPangToDirection(int inX, int inY, int offX, int offY, int count = 10, int effect = 0, float waitTime = 0.030f, List<BombAreaInfo> infoList = null, bool bNoneDirection = false)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            while (tempTimer < waitTime && GameManager.instance.SkipIngameClear == false)
            {
