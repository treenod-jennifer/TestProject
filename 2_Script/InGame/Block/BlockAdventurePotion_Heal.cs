...
public class BlockAdventurePotion_Heal : BlockBase
{
    const string SpriteName = "potion_Heal";
    ...
    IEnumerator CoPangFinal()
    {
        ...
        while (timer < 1f)
        {
            mainSprite.transform.localScale = Vector3.one * 1.2f * ManagerBlock.instance._curveTreeScaleIn.Evaluate(timer);
            timer += Global.deltaTimePuzzle * 2.0f;
        ...
        {
            AdventureManager.instance.GetHealPotion(0.20f);
        }
        DestroyBlockData();
