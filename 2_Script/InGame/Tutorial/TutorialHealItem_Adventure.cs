...
public class TutorialHealItem_Adventure : TutorialBase
{
    IEnumerator Start()
    {
        ...
        #region 첫번째 연출 시작 (아이템이 도움이 될 거야)
        {
            //블라인드 설정.
        ...
        #endregion 첫번째 연출 끝 (아이템이 도움이 될 거야)

        #region 두번째 연출 시작 (동료를 부활시킬 수 있어)
        {
            //인겜 아이템
            GameObject HealObj = null;
            foreach (var item in GameUIManager.instance.listGameItem)
            {
                if (item.type == GameItemType.HEAL_ONE_ANIMAL)
                {
                    HealObj = NGUITools.AddChild(blind._panel.gameObject, item.gameObject);
                    HealObj.transform.position = item.gameObject.transform.position;
                    BoxCollider collider = HealObj.GetComponent<BoxCollider>();
                    Destroy(collider);
                    break;
                }
            }
