...
public class ActionCharacterSpeechBubble : ActionBase
{
    ...
    IEnumerator CoDoAction()
    {
        ...
        if ( _randomSpeechMode )
        {
            ...
            _lobbyChat = UILobbyChat.MakeLobbyChat(character._transform, Global._instance.GetString(selected._text), selected._dulation);
        ...
        else
        {
            for (int i = 0; i < _stringData.Count; i++)
            {
                ...
                _lobbyChat = UILobbyChat.MakeLobbyChat(character._transform, Global._instance.GetString(_stringData[i]._text), _stringData[i]._dulation);
                _lobbyChat.heightOffset = character.GetBubbleHeightOffset();
