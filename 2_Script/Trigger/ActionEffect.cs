...
public class ActionEffect : ActionBase
{
  ...
  //      Debug.Log("CoEffect _ DoAction");
                ...
                if (!_animation[i].play)
                {
                    ...
                    if (_animation[i].timer >= _animation[i]._delay)
                    {
                        ...
                        if (_animation[i]._particle != null)
                        {
                            ...
                            speedMultiplier = Global.timeScaleLobby;

                            if (_animation[i]._delayStop > 0f)
                            {
                                var allParticles = _animation[i]._localParticle.GetComponentsInChildren<ParticleSystem>();
                                for (int j = 0; j < allParticles.Length; ++j)
                                {
                                    var particleMain = allParticles[j].main;
                                    particleMain.simulationSpeed = Global.timeScaleLobby;
                                }
                            }
                            else
                            {
    ...
    private void Update()
    {
        if( Global.timeScaleLobby != speedMultiplier)
        {
            speedMultiplier = Global.timeScaleLobby;
