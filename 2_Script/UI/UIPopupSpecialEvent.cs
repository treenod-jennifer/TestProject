...
{
    public static UIPopupSpecialEvent _instance = null;
    ...
    public void InitPopUp(int index, bool displayStartButton, Protocol.AppliedRewardSet rewardSet)
    {
        SettingLoad();
        ...
        ItemCountIcon.SuccessEvent += SettingIcon;
        ItemCountIcon.LoadCDN(Global.gameImageDirectory, "IconEvent/", iconName);
        ...
        if (displayStartButton == false)
            this.startButton.gameObject.SetActive(false);

        //보상받았어! 팝업
        if (PlayerPrefs.HasKey("ShowSpeicalEventPopup") == true)
            StartCoroutine(CoOpenRewardAlarmPopup());
    }
    ...
    private IEnumerator CoOpenRewardAlarmPopup()
    {
        PlayerPrefs.DeleteKey("ShowSpeicalEventPopup");

        int prevRewardIdx = (userIndex == -1) ? -1 : ServerRepos.UserSpecilEvents[userIndex].rewardSection - 1;
        if (prevRewardIdx < 0)
            yield break;
