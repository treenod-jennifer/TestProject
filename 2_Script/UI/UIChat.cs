...
public class UIChat : MonoBehaviour
{
    ...
    private Dictionary<string, Texture2D> chatBG = new Dictionary<string, Texture2D>();
    ...
    {
        int loadedCount = 0;
        int completeCount = _chatGameData._listChatData.Count * 3;

        foreach(var chatData in _chatGameData._listChatData)
        {
            SetTexture(chatData.chatBack_0, () => Complete(++loadedCount, completeCount));
            SetTexture(chatData.chatBack_1, () => Complete(++loadedCount, completeCount));
            SetTexture(chatData.chatBackCenter, () => Complete(++loadedCount, completeCount));
        }

        void Complete(int loaded, int complete)
        {
            if(loaded == complete)
            {
                SetChatAnchor();
        ...
        void SetTexture(string fileName, System.Action complete)
        {
            ...
            string fullPath = ChatGameData.GetFullPath(fileName);
            var cdnPath = StringHelper.GetCDNPath(fullPath);

            if (cdnPath.isSuccess)
            {
                Box.LoadCDN<Texture2D>(cdnPath.folderName, cdnPath.fileName, (texture) =>
                {
                    if (!chatBG.ContainsKey(fileName))
                    {
                        chatBG.Add(fileName, texture);
                    }
    ...
    public void ShowChatUI(bool bShow, ChatGameData chatGameData)
    {
        ...
        if (bShow == true)
        {
            ...
            StartCoroutine(CoAction(_startTime, () =>
            {
                ...
                {
                    AddDialog(chatData.boxType, Global._instance.GetString(chatData.stringKey), chatData.bChatColor);
    ...
    public void CloseBtn()
    {
        ...
        if (bEndChatUIAct == true)
        {
            bEndChatUIAct = false;
            //마지막 대사가 아닐 경우.
            if (currentChatDataIndex < _listChatData.Count && !bSkip)
            {
    ...
    public void Skip()
    {
        if (skipCalled == true || chatData == null || _listChatData == null)
            return;
    ...
    IEnumerator SkipBtn()
    {
        ...
        if (currentChatDataIndex < _listChatData.Count)
        {
            ...
            if (continueChat)
            {
                ...
                {
                    chatData.conditionData.gameObject.SetActive(true);
                    //Debug.LogFormat("SkipBtn ({0}) : chatData Cond On {1}", Time.frameCount, chatData.conditionData.gameObject.name);
                }
                ...
                yield return null;
                //스킵;
                yield return SkipBtn();
                yield break;
            }
        }

        //Debug.LogFormat("SkipBtn End ({0}) : chatData {1}", Time.frameCount, gameObject.name);
        ...
        StartCoroutine(CoAction(addDialogTime, () =>
        {
            if (strChat != "")
            {
                ...
                else if (boxType == TypeChatBoxType.Mission && missionSpriteCnt > 0)
                {
                    ...
                    missionUI.InitMissionSprite(Global._instance.GetString(key), missionData.index);
                ...
                else if (boxType == TypeChatBoxType.Monologue)
                {
                    Vector3 pos = new Vector3(0f, 280f, 0f);

                    UIItemDialogMonologue dialog = null;
                    dialog = ManagerUI._instance.InstantiateUIObject("UIPrefab/UIItemDialogMonologue", scrollPanel.gameObject).GetComponent<UIItemDialogMonologue>();
                    dialog.transform.localPosition = pos;
                    dialog.InitDialogBubble(strChat);
                    _ListDialogs.Add(dialog);
                    DIALOG_SPACE_SIZE = dialog._sprDialogBox.height + 10;
    ...
    {
        chatBG.TryGetValue(chatData.chatBackCenter, out Texture2D textureCenter);
    ...
    private IEnumerator SetFirstCallBackGround(float startTime)
    {
        ...
        chatBG.TryGetValue(chatData.chatBack_0, out Texture2D textureLeft);
        chatBG.TryGetValue(chatData.chatBack_1, out Texture2D textureRight);
    ...
    private void OnClickSkip()
    {
        ...
        for( int i = currentChatDataIndex; i < _listChatData.Count; ++i)
        {
            var cData = _listChatData[i];

            if (cData.addMission != null && cData.addMission.Count > 0)
            {
                for (int j = 0; j < cData.addMission.Count; ++j)
                {
                    ManagerSound.AudioPlay(AudioLobby.CreateMission);
                    Vector3 pos = new Vector3(0f, 175f, 0f);
                    UIItemDialogMission missionUI = NGUITools.AddChild(scrollPanel.gameObject, _objDialogMission).GetComponent<UIItemDialogMission>();
                    ...
                    //미션 데이터 가져오기.
                    MissionData missionData = ManagerData._instance._missionData[cData.addMission[j]._missionIndex];
                    string key = "m" + missionData.index;
                    missionUI.InitMissionSprite(Global._instance.GetString(key), missionData.index);
                    _ListDialogs.Add(missionUI);
                    ...
                    DIALOG_SPACE_SIZE = missionUI._sprDialogBox.height + 10;
                    //대화 중 미션이 있으면 월드에 미션 아이콘 추가
                    ManagerLobby._instance.OpenPopupLobbyMission(cData.addMission[j]._missionIndex, cData.addMission[j]._targetTranform.position + cData.addMission[j]._offset);
                }
            }
        }
