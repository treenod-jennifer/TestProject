...
public class UIPopupMissionStampEvent : UIPopupBase
{
    ...
    public override void OpenPopUp(int depth)
    {
        ...
        texture.Load(Global.gameImageDirectory, "IconEvent/", "line_stamp_" + eventIndex.ToString() + "_p");
    }
...
*/
    ...
    {/*
        ...
        if (day > 0)
        {
            text += day + (jp == true ? "日 " : "DAY");
        }

        text += string.Format("{0:D2}", (int)((t / 3600) % 24)) + (jp == true ? "時間 " : ": ");//"時間 ";
        text += string.Format("{0:D2}", (int)((t / 60) % 60)) + (jp == true ? "分 " : ": ");//"分 ";
        text += string.Format("{0:D2}", (int)(t % 60)) + (jp == true ? "秒": "");//+ "秒";
