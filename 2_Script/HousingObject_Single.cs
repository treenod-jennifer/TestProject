...
public class HousingObject_Single: MonoBehaviour, IHousingTrigger
{
    ...
    [System.NonSerialized] public bool _haveIndex = true;
    ...
    public Vector3 GetHousingFocusPosition()
    {
        if (_finishObject.Count > 0)
            return _finishObject[0].GetFocusPosition();
        else
    ...
    {
        return this.currentObject as ObjectEvent;
    }
    ...
    public void CloneFromLegacy(ActionObjectHousing legacyObject)
    {
        _housingIndex = legacyObject._housingIndex;
        _waitObject = legacyObject._waitObject;
        _finishObject = new List<ObjectBase>(legacyObject._finishObject);

        _objectList = new Dictionary<int, ObjectBase>(legacyObject._objectList);

        
        _iconOffset = legacyObject._iconOffset;
    ...
    public void MakeObject(int in_index, GameObject in_object)
    {
        try
        {
            ...
            AddObjectToList(in_index, model);
            ...
            model.OnCompleteMakeObject?.Invoke();
        }
    ...
    public void DoAction()
    {
        ...
        if (_housingIndex > 0 && _modelIndex > 0)
        {
            ...
            if (item.housingIndex == _housingIndex && item.active == 1)
            {
                ...
                if (item.type == PlusHousingModelDataType.byProgress || item.type == PlusHousingModelDataType.byPackage)
                {
                    ...
                    else
                    {
                        if (Global.LoadFromInternal)
                        {
                            ...
                            model = Instantiate<GameObject>(BundleObject).GetComponent<ObjectBase>();
#endif
                        }
                        else
                        {
                            string name = "h_" + _housingIndex + "_" + _modelIndex;
                            model = Instantiate<GameObject>(ManagerLobby._assetBankHousing[name]).GetComponent<ObjectBase>();
                        }
                        ...
                        if (model != null)
                        {
                            ...
                            _finishObject.Add(model);
                            AddObjectToList(_modelIndex, model);
                        }
                    }
                }
                ...
                if (model != null)
                {
                    if (currentObject != null && currentObject != model && model._transformModel != null)
                    {
                        for (int i = 0; i < currentObject.childObjects.Count; ++i)
                        {
                            currentObject.childObjects[i].transform.parent = model._transformModel;
                        }

                        model.childObjects = new List<GameObject>(currentObject.childObjects);
                        currentObject.childObjects.Clear();
                        model._callbackTapList = new List<TriggerState>(currentObject._callbackTapList);
                    }
        ...
        if (_modelIndex > 0 && _objectList.ContainsKey(_modelIndex))
        {
            ...
            if (_parentObj)
            {
                ...
                if (parentObj)
                {
                    ...
                    if (parentObj._housingDummyParent)
            ...
            if (prevObject != currentObject)
            {
                if (prevObject != null)
                {
                    for (int i = 0; i < prevObject.childObjects.Count; ++i)
                    {
                        prevObject.childObjects[i].transform.parent = currentObject._transformModel;
                    }
                    currentObject.childObjects = new List<GameObject>(prevObject.childObjects);
                    prevObject.childObjects.Clear();
                    currentObject._callbackTapList = new List<TriggerState>(prevObject._callbackTapList);
                }
    ...
    ObjectEvent GetCurrentParentObj()
    {
        ...
        if (_parentObj.housingIdx != 0)
        {
            ...
            return parentHousing.GetCurrentMatchedObject(_parentObj) as ObjectEvent;
        }
...
public interface IHousingTrigger
{
    void MakeObject(int in_index, GameObject in_object);
    void BuyNewHousing();
    int GetHousingIdx();
    bool IsHousingActivated();

    void SelectModel(int modelIndex);
    void MakeHousingChat(int modelIdx);

    Vector3 GetHousingFocusPosition();
    ObjectEvent GetCurrentMatchedObject(ObjectEvent org);

    bool IsMovableHousing();
}