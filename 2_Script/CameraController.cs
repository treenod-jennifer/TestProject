...
public class CameraController : MonoBehaviour
{
    ...
    // farZoom value setting
    private const int insideFarZoom = 30;
    private const int outsideFarZoom = 40;
    
    // zoom
    private int NearZoom = 10;
    private int FarZoom = outsideFarZoom;
    ...
    {
        var farZoom = outsideFarZoom;

        //í˜„ìž¬ëŠ” ì‹¤ë‚´(landIndex == 100)ì¼ ë•Œ ë§Œ ì¤Œì„ ê¸°ì¡´(30)ìœ¼ë¡œ ìœ ì§€.
        //ì¶”ê°€ë¡œ ë‹¤ë¥¸ ëžœë“œê°€ ì¶”ê°€ì ìœ¼ë¡œ ì¤Œì•„ì›ƒì˜ ê°’ì´ ì ì–´ì§ˆ ë•Œ ì¡°ê±´ ì¶”ê°€.
        if (ManagerLobby.landIndex == 100)
            farZoom = insideFarZoom;
    ...
    void Update()
    {
        ...
        if( _rigidSkipTimer + Global.deltaTime > 0)
        {   // °­Á¦ Ä«¸Þ¶ó¿öÅ© ÀÌÈÄ, Ä«¸Þ¶ó°¡ ÀÌÀü ÀÚ¸®·Î µ¹¾Æ°¡·Á°í ÇÏ´Â ¹®Á¦ ¼öÁ¤
            AbortCameraTarget();
        }
    ...
    public void AbortCameraTarget()
    {
        _cameraTarget.transform.position = GetCenterWorldPos();
    }
    ...
    private void UpdateCameraPosition()
    {
        ...
        moveMaxSpeed = 1000f;
#endif
        move = BoundaryRayCasting(position, move);
    ...
    private void UpdateLobbyUI(Vector3 move)
    {
        if (ManagerUI._instance == null) return;
        if (move.magnitude > 0)
            ManagerUI._instance.UIState = UIState.HIDE;
        else if (move.magnitude <= 0)
            ManagerUI._instance.UIState = UIState.SHOW;
    }
    ...
    public void SetFieldOfView(float in_field)
    {
        ...
        moveCamera.orthographicSize = moveCamera.fieldOfView * 1.8f;
    }
    ...
    private int _cameraColliderLayer = 0;
    private int cameraColliderLayer
    {
        get
        {
            if (_cameraColliderLayer == 0)
                _cameraColliderLayer = LayerMask.GetMask("CameraCollider");

            return _cameraColliderLayer;
        }
    }
    private Vector3 BoundaryRayCasting(Vector3 origin, Vector3 direction)
    {
        RaycastHit hit;
        if (RayCasting(origin, direction, out hit))
        {
            Vector3 endPos = hit.point;
            Vector3 endDirection = endPos - origin;

            //Debug.DrawRay(origin, endDirection, Color.yellow);
            //Debug.DrawLine(origin + direction, endPos, Color.green);
    ...
    private bool RayCasting(Vector3 origin, Vector3 direction, out RaycastHit hit, float distance = 0.0f)
    {
        if (distance == 0.0f)
        ...
        if (Physics.Raycast(ray: ray, hitInfo: out hit, maxDistance: distance, layerMask: cameraColliderLayer))
            return true;
        else
            return false;
    }
