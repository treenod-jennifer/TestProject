...
public class TutorialGetPlusHousing : TutorialBase
{
    IEnumerator Start()
    {
        ...
        #region 네번째 연출 시작 (다이어리 창에서 창고 탭 누르기)
        {
            ...
            while (UIPopupDiary._instance == null || UIDiaryStorage._instance == null)
            {
                //기다리는 동안 손가락 이미지 전환.
                fTimer += Global.deltaTime * 1.0f;
                if (fTimer >= 0.15f)
        ...
        #region 일곱번째 연출 시작 (버튼 누르기)
        {
            ...
            GameObject getButton = benchItem.buttonGet.gameObject;
            ...
            getButton.gameObject.SetActive(false);
            getButton.transform.parent = benchItem.transform;
            getButton.gameObject.SetActive(true);*/
            yield return null;
        }
