...
public class TutorialReadyScoreUp : TutorialBase
{
    ...
    private void OnDisable()
    {
        var useReadyItem = new ServiceSDK.GrowthyCustomLog_ITEM
                (
                    ...
                    RewardType.readyItem2.ToString(),
                    "ReadyItem" + READY_ITEM_TYPE.SCORE_UP.ToString(),
                    ServerRepos.UserItem.ReadyItem(1) + 1,
    ...
    public GameObject GetObjReadyItem_ScoreUp()
    {
        for (int i = 0; i < UIPopupReady._instance.listReadyItem.Count; i++)
        {
            if (UIPopupReady._instance.listReadyItem[i].type == READY_ITEM_TYPE.SCORE_UP)
                return UIPopupReady._instance.listReadyItem[i].gameObject;
        }
        return null;
    }

    public List<GameObject> GetListObjReadyItem_ScoreUp()
    {
    ...
    public bool IsSelectReadyItem()
    {
        for (int i = 0; i < UIPopupReady._instance.listReadyItem.Count; i++)
        {
            ...
            if (readyItem.type == READY_ITEM_TYPE.SCORE_UP)
