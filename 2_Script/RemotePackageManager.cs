...
// KTW CHG
    ...
    public IEnumerator InstanceLoadSync(string packageUri, System.Action<AssetBundle> callback = null)
	{
	    ...
	    if (!bLocalLoad)
	    {
            ...
            using (var assetWWW = UnityWebRequestAssetBundle.GetAssetBundle(GetPackageUrl(packageUri)))
            {
                yield return assetWWW;
                if( assetWWW.IsError() || assetWWW.downloadHandler == null)
                {
                    yield break;
                }
                var dhBundle = (assetWWW.downloadHandler as DownloadHandlerAssetBundle);
                assetBundle = dhBundle.assetBundle;
                ...
                byte[] bytes = assetWWW.downloadHandler.data;
