...
[CustomEditor(typeof(EndContentsAreaBase), true)]
public class EditEndContentsAreaBase : Editor
{
    protected EndContentsAreaBase _target;

    protected void OnEnable()
    {
        wakeUpScene = PlayerPrefs.GetInt("EditEndContentsAreaBase", 0);
    }
    void ReAttach()
    {
        _target = target as EndContentsAreaBase;
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (beforeWakeUpScene != wakeUpScene)
            PlayerPrefs.SetInt("EditEndContentsAreaBase", wakeUpScene);
