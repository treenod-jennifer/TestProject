...
public class RemotePackageMakeFileListWindow : EditorWindow 
{
    ...
    {
        Dictionary<string, TownBundleCompareData> bundleData = new Dictionary<string, TownBundleCompareData>();
        ...
        for ( int i = 0; i < length; i++ )
        {
            ...
            if ( file.Name.IndexOf( ".manifest" ) != -1f )
            {
                ...
                TownBundleCompareData data = new TownBundleCompareData();
        ...
        for (int i = 0; i < length; i++)
        {
            ...
            {
                TownBundleCompareData data;
                if(bundleData.TryGetValue(file.Name, out data))
        ...
        if ( File.Exists( filePath ) )
        {
            ...
            File.WriteAllText( _findText + "/ChangeFileList.text",
                this.CompareChangeAssetBundleList( JsonFx.Json.JsonReader.Deserialize<Dictionary<string, TownBundleCompareData>>( originText ), bundleData ), 
    ...
    /// <returns></returns>
    private string CompareChangeAssetBundleList ( Dictionary<string, TownBundleCompareData> originDataList, Dictionary<string, TownBundleCompareData> changeBundleData )
    {
    ...
    private List<string> CompareData ( Dictionary<string, TownBundleCompareData> originDataList, Dictionary<string, TownBundleCompareData> changeBundleData )
    {
        // 원래의 데이터 키값들, ValueList들
        List<TownBundleCompareData> dataList = originDataList.Values.ToList();
