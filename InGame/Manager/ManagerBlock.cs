...
{ 
    DECO_BASE       = 5,                        // 데코 뎁스 디폴트 (5)
    DECO_AREA       = DECO_BASE + 10,           // 데코 뎁스 1 / 데코 
    DECO_LAND       = DECO_AREA + 10,           // 데코 뎁스 2 / 잔디 (고정 기믹 있을 때)
                                                   
    DECO_GROUND     = DECO_LAND + 10,           // 데코 뎁스 3 / plantPlace
    DECO_FIELD      = DECO_GROUND + 10,         // 데코 뎁스 4 / 잔디 (고정 기믹 없을 때)
    DECO_FLOOR      = DECO_FIELD + 10,          // 데코 뎁스 5 / 방해블럭1
    DECO_SURFACE    = DECO_FLOOR + 10,          // 데코 뎁스 6 / 하트길, 폭탄, 방해블럭2
    DECO_PLACE      = DECO_SURFACE + 10,        // 데코 뎁스 7 / plantPlace : 빵틀
    DECO_OBJECT     = DECO_PLACE + 10,          // 데코 뎁스 8 / 출발, 생성기
    DECO_UNDER      = DECO_OBJECT + 10,         // 데코 뎁스 9 / 물
                                                   
    BLOCK_BASE      = DECO_UNDER + 10,          // 블럭 뎁스 디폴트 
    BLOCK_AREA      = BLOCK_BASE + 10,          // 블럭 뎁스 1 / 벌집
    BLOCK_LAND      = BLOCK_AREA + 10,          // 블럭 뎁스 2 / 물, 물 위에 떠야 하는 기믹 (ex.째깍폭탄 카운트)
    BLOCK_GROUND    = BLOCK_LAND + 10,          // 블럭 뎁스 3 / 땅
...
public class ManagerBlock : MonoSingletonOnlyScene<ManagerBlock>
{
    ...
    public int SameColorProb = 0;
    #endregion

    #region 모험모드
    public float waitComboTimer = 0f;
    public int[] autoBlockLiveCount = new int[5];
    ...
    void ResetManager()
    {
        ...
        iceCount = 0;
        comboCount = 0;
        coins = GameManager.gameMode == GameMode.ADVENTURE || Global.GameType == GameType.SPACE_TRAVEL ? 0 : 10 ;
        ...
        GetWater = false;
        _portalPause = false;
        autoBlockLiveCount = new int[5] { 0, 0, 0, 0, 0 };
    }
    ...
    public void RefreshBlockManagerByStageData()
    {
        ...
        if (GameManager.gameMode == GameMode.ADVENTURE)
            PosHelper.Block_OffsetY = -10f + 70f * 6.3f - 50f;
        ...
        foreach (BlockInfo blockInfo in stageInfo.ListBlock)
        {
            ...
            else if (blockInfo.type == (int)BlockType.BLOCK_EVENT)
            {
                ...
                if (Global.GameInstance.GetProp(GameTypeProp.CAN_PLAY_COLLECT_EVENT) && ServerContents.EventChapters.active == 1 && ServerContents.EventChapters.type == (int)EVENT_CHAPTER_TYPE.COLLECT) //추가 이벤트 타입확인 //이벤트 시간확인 //매니져데이타에서
                {
                    ...
                    else
                    {
                        ...
                        {
                            if (ManagerBlock.hasDeco(blockInfo.ListDeco, decoType: new[] { BoardDecoType.ICE, BoardDecoType.NET }))
                            {
                ...
                else if (EditManager.instance != null)
                {
                    board.Block = BlockMaker.instance.MakeBlockBase(board.indexX, board.indexY, (BlockType)blockInfo.type, (BlockColorType)blockInfo.colorType, blockInfo.count, blockInfo.index, blockInfo.subType);
        ...
        for (int i = GameManager.MIN_X; i < GameManager.MAX_X; i++)
        {
            for (int j = GameManager.MIN_Y; j < GameManager.MAX_Y; j++)
            {
                if (!boards[i, j].IsActiveBoard && boards[i, j].IsHasScarp)
                {
                    ...
                    if (stageInfo.gameMode == (int)GameMode.ADVENTURE)
                        offsetY = -50f;
        ...
        //해당 스테이지에서 움직일 수 있는 턴 설정.
        Global.GameInstance.SetMoveCount_Ingame(stageInfo.gameMode, stageInfo.digCount, stageInfo.turnCount);
            ...
            */
            switch (state)
            {
                ...
                case BlockManagrState.BEFORE_WAIT:
                    if (checkBlockWait())
                    {
                        if (isPlayBeforeWaitAction == false)
                        {
                            isPlayBeforeWaitAction = true;
                            StartCoroutine(Global.GameInstance.GameModeProcess_OnBeforeWait());
                        }
                    }
                    else
                    {
    ...
    public IEnumerator CoWaitEnd_BeforeWaitAction()
    {
        // 대포 발사
    ...
    public void MakeBombRandom(BlockBase targetBlock, BlockBombType bomb = BlockBombType.NONE)
    {
        if (targetBlock == null)
            return;

        if (bomb == BlockBombType.NONE)
        {
            int randomLine = GameManager.instance.GetIngameRandom(0, 3);

            if (randomLine == 0) targetBlock.bombType = BlockBombType.BOMB;
            else if (randomLine == 1) targetBlock.bombType = BlockBombType.LINE_V;
            else targetBlock.bombType = BlockBombType.LINE_H;
        }
        else
        {
            targetBlock.bombType = bomb;
        }

        ManagerSound.AudioPlay(AudioInGame.CREAT_BOMB);
        InGameEffectMaker.instance.MakeBombMakeEffect(targetBlock._transform.position);
    ...
    void LateUpdate()
    {
        ...
        if (creatBlock)
        {
            if (GameManager.gameMode == GameMode.ADVENTURE && GameManager.adventureMode == AdventureMode.CURRENT )
            {
                return;
            }
            ...
            for (int i = 0; i < startBoardList.Count; i++)
            {
                    ...
                    && startBoardList[i].IsNotDisturbBlock(checkDirection))
                {
                    ...
                    {
                        if (startBoardList[i].Block.IsNormalBlock() && startBoardList[i].Block.type == BlockType.NORMAL
                            && specialEventCount < GameManager.instance.specialEventMaxCount && specialEventAppearCount < GameManager.instance.specialEventAppearMaxCount)
                        {
                    ...
                    {
                        if (startBoardList[i].Block.IsNormalBlock() && startBoardList[i].Block.type == BlockType.NORMAL
                            && ManagerAlphabetEvent.alphabetIngame.IsCanAppearAlphabetBlock_N())
                        {
    ...
    public bool checkBlockWait()
    {
        ...
        if (GameManager.instance.SkipIngameClear == false && BlockBase._waitCount == BlockBase._listBlock.Count && BlockBomb._liveCount == 0 && Time.frameCount - _lastFrame > 8)        
                return true;

        if (GameManager.instance.SkipIngameClear == true && BlockBase._waitCount == BlockBase._listBlock.Count && BlockBomb._liveCount == 0 && Time.frameCount - _lastFrame > 0)
            return true;
    ...
    [HideInInspector] public bool isPlayBeforeWaitAction = false;
    ...
    IEnumerator CoEventAction()
    {
        ...
        //알파벳 모으기 이벤트
        yield return CoAlphabetCollectEvent();
    ...
    bool SearchMatchBlock()
    {
        ...
        for (int y = GameManager.MinScreenY; y < GameManager.MaxScreenY; y++)
        {
            for (int x = GameManager.MinScreenX; x < GameManager.MaxScreenY; x++)
            {
                ...
                if (block != null && block.IsSelectable() && tempBoard != null && tempBoard.HasDecoHideBlock() == false && tempBoard.HasDecoCoverBlock() == false && (block.blockDeco == null || block.blockDeco.IsInterruptBlockSelect() == false))
                {
                    if(block.inGameItemUse == true)
                    {
                        return true;
                    }
                    
                    if(block.IsCanLink() == true)
                    {
    ...
    public void RecoverLavaMode(int continueCount)    //컨티뉴
    {
        ...
        if (LavaLevelCount != 0)
        {
            ...
            {
                GameManager.MOVE_Y -= LavaLevelCount; ;
                LavaLevelCount = 0;
                Vector3 targetY = new Vector3(0, (GameManager.MOVE_Y) * 78, 0);
                StartCoroutine(DownLava(targetY));
            }
        }
    ...
    void SetGameMode(GameMode gameMode)
    {
        ...
        else if (gameMode == GameMode.ADVENTURE && GameManager.adventureMode != AdventureMode.ORIGIN)
        {
            GameUIManager.instance.block_Panel.clipping = UIDrawCall.Clipping.SoftClip;
            GameUIManager.instance.block_Panel.clipOffset = new Vector2(0, -33);
    ...
    IEnumerator CoResetNoMoreMove()
    {
        ...
        if (gameFail)
        {
            ...
            else
            {
                ...
                GameManager.instance.StageFail(false, true);
            }
        }
        else
        {
    ...
    public void AddScore(int tempScore)
    {
        ...
        if ((Global.GameInstance.CanUseReadyItem(1) && UIPopupReady.readyItemUseCount[1].Value == 1)
            || (EditManager.instance != null && EditManager.instance.readyItemTest[1] == true))
        {
    ...
    #region 알파벳 모으기 이벤트
    private IEnumerator CoAlphabetCollectEvent()
    {
        //스페셜 알파벳이 등장할 수 있는지 검사.
        if (Global.GameInstance.GetProp(GameTypeProp.CAN_PLAY_ALPHABET_EVENT) == false
            ...
            || ManagerAlphabetEvent.instance.IsExistSpecialBlock() == false)
            yield break;

        //스페셜 알파벳이 등장할 수 있는 턴인지 검사.
        if (ManagerAlphabetEvent.alphabetIngame.IsCanAppearAlphabetBlock_S(GameManager.instance.moveCount) == false)
            yield break;

        //스페셜 알파벳 생성 연출
        yield return CoAppearSpecialAlpabetEvent();        
    }

    private IEnumerator CoAppearSpecialAlpabetEvent()
    {
        //블럭 중 스페셜 알파벳 붙일 수 있는 블럭 찾기
        for (int i = GameManager.MinScreenY; i < GameManager.MaxScreenY; i++)
        {
            for (int j = GameManager.MinScreenX; j < GameManager.MaxScreenX; j++)
            {
                Board back = PosHelper.GetBoardSreeen(j, i);
                if (back != null && !back.HasDecoHideBlock() && !back.HasDecoCoverBlock() && back.Block != null)
                {
                    BlockBase block = back.Block;
                    if (block.IsNormalBlock() && block.type == BlockType.NORMAL && block.collectBlock_Alphabet == null
                        && (block.blockDeco == null || !block.blockDeco.IsInterruptBlockSelect()))
                    {
                        ManagerAlphabetEvent.alphabetIngame.AppearMoveCount_S = 0;

                        //UI 갱신.
                        GameUIManager.instance.AppearAlphabetEvent_Special();

                        //블럭에 알파벳 붙는 연출.
                        int alphabetIdx = ManagerAlphabetEvent.alphabetIngame.Alphabet_S;
                        block.AddAlphabetEventBlock(alphabetIdx, false, false);
                        yield return block.CoActionChangeBlock_Appear();
                        yield break;
                    }
                }
            }
    ...
    void SetCoinStage()
    {
        ...
        GameUIManager.instance.InitCoinStage();
    ...
    public void MakeBlockRandomPos(int maxCount, BlockType type, int blockCount, bool isMakeActiveBoard = true)
    {
        ...
        if (stageInfo != null && stageInfo.heightDownOneSpace == 1)
        {
            maxY -= 1;
        }
    ...
    public void CheckFeverBlock(BlockColorType tempColor)
    {
        ...
        if (feverBlockCount >= FEVER_BLOCK_COUNT)
        {
            ...
            ManagerSound.AudioPlay(AudioInGame.FEVER_START);
    ...
    void UpdateCoinStage()
    {
        ...
        {
            if( GameManager.instance.moveCount <= 10 != bgmHurryOn )
            {
                ManagerSound._instance.StopBGM();
                bgmHurryOn = GameManager.instance.moveCount <= 10;
                ManagerSound.AudioPlay(AudioInGame.TIME_ALERT);
                ManagerSound._instance.PlayBGM();
    ...
    private IEnumerator NoyBoostingEvent ()
    {
        ...
        if (ManagerNoyBoostEvent.instance == null || GameManager.instance.boostingStep == 0)
            yield break;
        ...
        {
            int cnt = 0;
            while (cnt < GameManager.instance.boostingStep)
            {
    ...
    private IEnumerator CoChangeBlockAnotherType(bool isAction, BlockBase baseBlock, BlockType type, BlockColorType blockColorType, int count = 0, int index = 0, int subType = 0, List<DecoInfo> decoList = null, System.Action endAction = null)
    {
        ...
        if (baseBlock.collectBlock_Alphabet != null)
            changeBlock.AddAlphabetEventBlock(baseBlock.collectBlock_Alphabet);
    ...
    public void AddGimmickDataByBlockType(UseGimmickData gimmickData, int blockType, int subType = 0, int bombType = 0)
    {
        ...
        switch ((BlockType)blockType)
        {
            ...
            case BlockType.WATERBOMB:
                listBlockType.Add(BlockType.WATERBOMB);
                subDecoType = BoardDecoType.WATER;
                break;
            #endregion
