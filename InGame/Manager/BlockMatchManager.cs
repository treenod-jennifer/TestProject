...
public class BlockMatchManager : MonoSingletonOnlyScene<BlockMatchManager>
{
    ...
    {        
        ...
        for (int i = 0; i < checkBlocks.Count; i++)
        {
            if (checkBlocks[i].state != BlockState.PANG)
            {
                if (!changeState)
                {
                    Board checkBoard = PosHelper.GetBoard(checkBlocks[i].indexX, checkBlocks[i].indexY);
                    if(checkBoard.HasDecoHideBlock() == false && checkBoard.HasDecoCoverBlock() == false)
                    {
                        checkBlocks[i]._pangRemoveDelay = 0.4f;

                        if (checkBlocks[i] != block)
                            checkBlocks[i]._pangGatherEffect = block._transform.localPosition;
                    }
                }
                if (GameManager.gameMode == GameMode.ADVENTURE)
                {
                ...
                checkBlocks[i].RemoveLinkerNoReset();
                //checkBlocks[i].pangIndex = BlockBomb._bombUniqueIndex;
                //if (checkBlocks[i].isMakeBomb && HasDynamite) checkBlocks[i].pangIndex = BlockBomb._bombUniqueIndex + 1;
    ...
    public bool toyBlastCheckMatchSpecialblock(BlockBase block)
    {
        ...
        if (!block.IsBombBlock() && block.colorType != BlockColorType.NONE && block.state == BlockState.WAIT)
        {
            ...
            if (GameManager.gameMode == GameMode.ADVENTURE && tempCount >= ManagerBlock.ADVENTURE_PANG_SAME_COLOR_COUNT)
            {
                creatItem = BlockBombType.NONE;

                //if (ManagerBlock.instance.state == BlockManagrState.MOVE)
                {
                    ManagerBlock.instance.comboCount++;
                    InGameEffectMaker.instance.MakeAdventureCombo(block._transform.position, "Combo " + ManagerBlock.instance.comboCount.ToString());// + new Vector3(0, -0.06f, 0)

                    if (GameManager.gameMode == GameMode.ADVENTURE)
                        AdventureManager.instance.GetAdventureGaige(8);
                    // GameUIManager.instance.SetAdvantureGaige(3);
