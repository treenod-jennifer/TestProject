...
public class NormalBlock : BlockBase
{
    ...
    public override void PostBlockMake()
    {
        base.PostBlockMake();
        if (bombType == BlockBombType.RAINBOW && rainbowSprite != null)
            ManagerUIAtlas.CheckAndApplyEventAtlas(rainbowSprite);
    ...
    public override bool BlockPang(int tempPangIndex = 0, BlockColorType pangColorType = BlockColorType.NONE, bool PangByBomb = false, bool isPangByPowerBomb = false)
    {
        ...
        if (isMakeBomb)
        {
            ...
            if (IsAutoBlock == false) InGameEffectMaker.instance.MakeBombMakeEffect(_transform.position);
    ...
    public override void Pang(BlockColorType pangColorType = BlockColorType.NONE, bool PangByBomb = false)
    {
        ...
        if (IsAutoBlock)
        {
            int tempAutoColorCount = (int)colorType - 1;
            ManagerBlock.instance.autoBlockLiveCount[tempAutoColorCount]--;
        }
        ...
        if (bombType != BlockBombType.NONE && bombType != BlockBombType.HALF_BOMB && bombType != BlockBombType.CLEAR_BOMB && !secendBomb)
        {
            switch (bombType)
            {
                ...
                case BlockBombType.BOMB_X_BOMB:
                    ManagerBlock.instance.creatBombCount[3].Value++;
                    break;
            } 
        }
        ...
        //if (Global.SkipIngameClear == false)
        {
            if (isUseMakeBomb)
            {
        ...
        else if(bombType != BlockBombType.NONE && inGameItemUse)
        {
            ManagerBlock.instance.PangColorBlock(colorType, transform.position);
        }
    ...
    public override string GetSpriteName()
    {
        if (bombType == BlockBombType.DUMMY)
        {
            switch (TempBombType)
            {
                ...
                default:
                    if(IsAutoBlock)
                        return "block" + GetColorTypeString() + "Auto";
    ...
    float makeBombWaitTimer = 0;

    void ShowMakeBombEffect()
    {
        if (IsAutoBlock && isShowMakeBombEffect)
        {
            makeBombWaitTimer += Global.deltaTimePuzzle;
            mainSprite.cachedTransform.localEulerAngles = new Vector3(0, 0, 1500 * ManagerBlock.instance.BlockTime);

            if (makeBombWaitTimer > 0.8f)
            {
                IsAutoBlock = false;
                makeBombWaitTimer = 0f;
                mainSprite.cachedTransform.localEulerAngles = Vector3.zero;
    ...
    public override void MakeDummyImage(Vector3 tartgetPos)
    {
        ...
        if (specialEventSprite != null)
        {
            ...
            specialEventSpriteA.LoadCDN(Global.gameImageDirectory, "IconEvent/", "sEventBlock_" + Global.specialEventIndex);
        ...
        if (collectBlock_Alphabet != null)
        {
            CollectBlock_Alphabet cloneAlphabetBlock 
                = NGUITools.AddChild(mainSprite.gameObject, BlockMaker.instance.alphabetEventObj).GetComponent<CollectBlock_Alphabet>();

            cloneAlphabetBlock.InitCollectBlock(collectBlock_Alphabet);
            dummySpriteList.Add(cloneAlphabetBlock.GetCustomFill());
        }
    }
    ...
    public override void SpriteRatio(float ratio, float vertRatio = 0)
    {
        ...
        if (collectBlock_Alphabet != null)
        {
            collectBlock_Alphabet.GetCustomFill().verticalRatio = vertRatio;
            collectBlock_Alphabet.GetCustomFill().blockRatio = ratio;
        }
    }
