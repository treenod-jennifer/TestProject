...
public class TutorialBombXBomb : TutorialBase
{
    IEnumerator Start()
    {
        float timer = 0;
        ...
        #region 첫번째 연출 시작 
        {
            targetPos = (ManagerBlock.boards[8, 9].Block.transform.position + ManagerBlock.boards[8, 10].Block.transform.position) * 0.5f;
            ...
            //말풍선 생성.
            textBox = ManagerTutorial._instance.MakeTextbox(birdType, Global._instance.GetString("t_g_m5_1"), 0.3f);      //더 강한 효과를 보고 싶다면            폭탄을 연결해서 사용해 봐.

            //손가락 생성.
            finger.color = new Color(1f, 1f, 1f, 0f);
            DOTween.ToAlpha(() => finger.color, x => finger.color = x, 1f, 0.2f);
            finger.transform.DOMove(ManagerBlock.boards[8, 10].Block.transform.position, 0.3f);
            blind.SetSizeCollider(78 , 78*2);
        ...
        #region 
        {
            ...
            //말풍선 생성.
            textBox = ManagerTutorial._instance.MakeTextbox(birdType, Global._instance.GetString("t_g_m5_2"), 0.3f);        //어때마유? 폭탄 조합을 터트리니       효과가 어마어마하지?
            ...
            ManagerSound.AudioPlay(AudioLobby.m_bird_hehe);
            textBox = ManagerTutorial._instance.MakeTextbox(birdType, Global._instance.GetString("t_g_m5_3"), 0.3f);        //"폭탄의 종류에 따라 다양하게     조합되니까, 많이 만들어 봐마유!
