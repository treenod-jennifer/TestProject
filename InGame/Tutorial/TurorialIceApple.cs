...
public class TurorialIceApple : TutorialBase
{
    ...
    IEnumerator Start()
    {
        ...
        #region 연출 시작
        {
            targetPos = (ManagerBlock.boards[3, 9].Block.transform.position + ManagerBlock.boards[4, 5].Block.transform.position) * 0.5f;
            ...
            listBlock.Add(ManagerBlock.boards[3, 9].Block as BlockBase);
            ...
            //말풍선 생성.
            textBox = ManagerTutorial._instance.MakeTextbox(birdType, Global._instance.GetString("t_g_m11_1"), 0.3f);      //더 강한 효과를 보고 싶다면            폭탄을 연결해서 사용해 봐.
        ...
        #region 첫번째 연출 시작
        {
            targetPos = (ManagerBlock.boards[3, 10].Block.transform.position + ManagerBlock.boards[2, 10].Block.transform.position) * 0.5f;
            ...
            //블럭이미지복사
            List<BlockBase> listBlock = new List<BlockBase>();
            listBlock.Add(ManagerBlock.boards[2, 10].Block as BlockBase);
            listBlock.Add(ManagerBlock.boards[3, 10].Block as BlockBase);
            ...
            //말풍선 생성.
            textBox = ManagerTutorial._instance.MakeTextbox(birdType, Global._instance.GetString("t_g_m11_2"), 0.3f);      //더 강한 효과를 보고 싶다면            폭탄을 연결해서 사용해 봐.
            timer = 0;
            while (timer < 0.5f)
            {
                timer += Global.deltaTimePuzzle;
