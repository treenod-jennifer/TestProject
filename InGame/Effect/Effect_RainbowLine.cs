...
public class Effect_RainbowLine : MonoBehaviour
{
    ...
    public bool AdventureRainbowXRainbow = false;
    ...
    {     
        ...
        if (tempBlock != null)
        {
            ...
            if (bombType != BlockBombType.NONE)
            {
                if (isCanPangBlock == true)
                {
                    ...
                    {
                        AdventureManager.instance.AddAnimalAttack(tempBlock.colorType, tempBlock._transform.position);
                    }
        ...
        if(tempBlock != null)
        {
            ...
            if (GameManager.gameMode == GameMode.ADVENTURE &&
                AdventureRainbowXRainbow &&
                bombType == BlockBombType.NONE &&
                tempBlock.bombType != BlockBombType.DUMMY &&
                tempBlock.bombType != BlockBombType.RAINBOW_X_RAINBOW &&
                tempBlock.state != BlockState.PANG &&
                tempBlock.secendBomb == false &&
                tempBlock.IsBombBlock())
            {
                //블럭이 삭제 가능한지 검사.
                if (isCanPangBlock == true)
                {
                    ManagerBlock.instance.comboCount++;
                    InGameEffectMaker.instance.MakeAdventureCombo(tempBlock._transform.position, "Bomb\nCombo " + ManagerBlock.instance.comboCount.ToString());// + new Vector3(0, -0.06f, 0)
                }
            }
